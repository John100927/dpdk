...
static int FLAGS_write_buffer_size = 64 * 1024 * 1024;
...
class NetworkClient {
  public:
      NetworkClient(const std::string& server_ip, int port) {
          ...
          if (connect(sockfd_, (struct sockaddr*)&server_addr_, sizeof(server_addr_)) < 0) {
              perror("Connection to F-Stack server failed");
              exit(1);
          }
      }
  
      std::string SendCommand(const std::string& command) {
          send(sockfd_, command.c_str(), command.size(), 0);
...
class Benchmark {
 ...
 private:
  ...
  void DoWrite(ThreadState* thread, bool seq) {
    ...
    // 創建 TCP 連線
    NetworkClient client("140.120.15.172", 9000);

    for (int i = 0; i < num_; i++) {
        const int k = seq ? i : thread->rand.Uniform(FLAGS_num);

        // ✅ 確保 key 為 16-byte
        char formatted_key[17];  
        snprintf(formatted_key, sizeof(formatted_key), "%016d", k);
        std::string clean_key = formatted_key;

        std::string value = gen.Generate(value_size_).ToString();

        // 🔹 構造 `PUT` 指令 (移除 `\n`)
        std::string command = "PUT " + clean_key + " " + value;

        // 🔹 送出 `PUT`
        std::string response = client.SendCommand(command);

        // 🔹 檢查回應
        if (response.find("PUT SUCCESS") == std::string::npos) {
            std::fprintf(stderr, "PUT Failed: %s\n", response.c_str());
            std::exit(1);
        }

        bytes += value_size_ + clean_key.size();
        thread->stats.FinishedSingleOp();
    }

    // ✅ 使用 `CompactRange()` 來確保 `readrandom` 讀取到最新數據
    if (db_ != nullptr) {
        this->db_->CompactRange(nullptr, nullptr);
    } else {
        printf("❌ ERROR: db_ 未初始化，無法執行 CompactRange\n");
    }
...
void ReadSequential(ThreadState* thread) {
  NetworkClient client("140.120.15.172", 9000);  // 連接 F-Stack 伺服器
  std::string request = "READSEQ";
  std::string response = client.SendCommand(request);
  ...
  while (std::getline(ss, line) && i < reads_) {
...
void ReadReverse(ThreadState* thread) {
  NetworkClient client("140.120.15.172", 9000);  // 連接 F-Stack 伺服器
  std::string request = "READREVERSE";
  std::string response = client.SendCommand(request);
...
void ReadRandom(ThreadState* thread) {
  ...
  for (int i = 0; i < reads_; i++) {
      ...
      // ✅ 確保 key 為 16-byte (前導零格式)
      char formatted_key[17];  // 16 bytes + null terminator
      snprintf(formatted_key, sizeof(formatted_key), "%016d", k);
      std::string clean_key = formatted_key;

      // ✅ 構造 `GET` 指令
      std::string command = "GET " + clean_key;

      // 透過 TCP 送出 `GET` 指令
      std::string response = client.SendCommand(command);

      // ✅ 檢查伺服器回應，確認是否成功找到 Key
      // printf("📩 伺服器回應: '%s'\n", response.c_str());

      if (response.find("GET SUCCESS") != std::string::npos) {  // 🔹 修正判斷條件
          found++;
      }
